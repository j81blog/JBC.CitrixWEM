function Convert-ShortcutPath {
    <#
    .SYNOPSIS
        Converts GPP shortcut path to WEM-compatible path format.

    .DESCRIPTION
        This helper function converts Group Policy Preferences (GPP) shortcut paths containing
        environment variables (like %StartMenuDir%, %DesktopDir%, etc.) to Citrix WEM-compatible
        path formats. It also sets appropriate location flags (IsAutoStart, IsDesktop, IsStartMenu)
        based on the path.

    .PARAMETER ShortcutPath
        The GPP shortcut path to convert (may contain environment variables).

    .PARAMETER ShortcutName
        The name of the shortcut (used for path normalization and warnings).

    .PARAMETER IsAutoStart
        Reference variable that will be set to $true if the shortcut is an autostart item.

    .PARAMETER IsDesktop
        Reference variable that will be set to $true if the shortcut is a desktop item.

    .PARAMETER IsStartMenu
        Reference variable that will be set to $true if the shortcut is a start menu item.

    .OUTPUTS
        Hashtable with two properties:
        - StartMenuPath: The converted WEM-compatible path
        - CreateShortcut: Boolean indicating if the shortcut should be created

    .EXAMPLE
        $result = Convert-ShortcutPath -ShortcutPath "%StartMenuDir%\MyApp.lnk" `
            -ShortcutName "MyApp" `
            -IsAutoStart ([ref]$isAuto) `
            -IsDesktop ([ref]$isDesk) `
            -IsStartMenu ([ref]$isStart)

    .NOTES
        Used internally by Get-GppShortcut function.
        Path mapping:
        - %StartMenuDir%  -> Start Menu\Programs
        - %DesktopDir%    -> Start Menu\Programs (desktop mapped to start menu)
        - %ProgramsDir%   -> Start Menu\Programs
        - %StartUpDir%    -> Start Menu\Programs\Startup
        - %SendToDir%     -> AppData\Roaming\Microsoft\Windows\SendTo
    #>
    [CmdletBinding()]
    param(
    ...
    # Path mapping for environment variables
    $PathMap = @{
        '%StartMenuDir%' = 'Start Menu\Programs'
        '%DesktopDir%'   = 'Start Menu\Programs'  # Desktop shortcuts mapped to Start Menu
        '%ProgramsDir%'  = 'Start Menu\Programs'
        '%StartUpDir%'   = 'Start Menu\Programs\Startup'
        '%SendToDir%'    = 'AppData\Roaming\Microsoft\Windows\SendTo'
    }
    ...
    $CreateShortcut = $true
    $StartMenuPath = $ShortcutPath

    # Find matching environment variable pattern
    foreach ($key in $PathMap.Keys) {
        if ($ShortcutPath -like "$key\*" -or $ShortcutPath -eq $key) {
            $StartMenuPath = $ShortcutPath -replace [regex]::Escape($key), $PathMap[$key]

            # Set location flags
            switch ($key) {
                '%StartMenuDir%' { $IsStartMenu.Value = $true }
                '%DesktopDir%'   { $IsDesktop.Value = $true }
                '%ProgramsDir%'  { $IsStartMenu.Value = $true }
                '%StartUpDir%'   { $IsAutoStart.Value = $true }
                '%SendToDir%'    { $CreateShortcut = $false }
            }
            break
        }
    }

    # If no match found, warn and disable creation
    if ($StartMenuPath -eq $ShortcutPath -and $ShortcutPath -match '%\w+%') {
        Write-Warning "Shortcut: `"$ShortcutName`", Unrecognized shortcut path:`r`n         '$ShortcutPath'."
        $CreateShortcut = $false
    }

    # Remove the shortcut name from the path
    $pattern = '\\{0}$' -f ($ShortcutName -replace '([\\\^\$\.\|\?\*\+\(\)\[\]\{\}])', '\$1')
    $StartMenuPath = $StartMenuPath -replace $pattern, ''

    # Normalize Start Menu paths
    if ($StartMenuPath -ieq "Start Menu" -or $StartMenuPath -ieq "Start Menu\" -or
        $StartMenuPath -ieq "Start Menu\P" -or $IsDesktop.Value) {
        $StartMenuPath = "Start Menu\Programs"
    }

    # Fix truncated "Programs" folder name
    if ($StartMenuPath -like "Start Menu\Prog*" -and $StartMenuPath -notlike "Start Menu\Programs*") {
        $parts = $StartMenuPath -split '\\'
        $parts[0] = "Start Menu"
        $parts[1] = "Programs"
        $StartMenuPath = ($parts -join '\')
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC