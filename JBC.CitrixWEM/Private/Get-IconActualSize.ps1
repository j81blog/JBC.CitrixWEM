function Get-IconActualSize {
    <#
    .SYNOPSIS
        Private helper function to get the actual size of an ICO file.

    .DESCRIPTION
        This internal function determines the actual dimensions of an ICO file by loading it as an image.
        This is useful because System.Drawing.Icon defaults to 32x32 when no size is specified,
        even if the ICO file contains different sizes.

    .PARAMETER FilePath
        The full path to the ICO file.

    .NOTES
        This is a private helper function used by Export-FileIcon.
        Returns the width (which equals height for square icons).
    #>
    [CmdletBinding()]
    param (
    ...
    try {
        # Read ICO file header to find the largest icon size
        # ICO format: 2 bytes reserved, 2 bytes type, 2 bytes count, then 16 bytes per entry
        $bytes = [System.IO.File]::ReadAllBytes($FilePath)

        if ($bytes.Length -lt 6) {
            Write-Verbose "File too small to be a valid ICO, defaulting to 32"
            return 32
        }
        ...
        if ($reserved -ne 0 -or $type -ne 1) {
            Write-Verbose "Not a valid ICO file, defaulting to 32"
            return 32
        }

        # Parse each entry to find the largest size
        $maxSize = 0
        for ($i = 0; $i -lt $count; $i++) {
            ...
            # Track the largest dimension found
            if ($width -gt $maxSize) { $maxSize = $width }
        }

        if ($maxSize -eq 0) {
            Write-Verbose "Could not parse ICO entries, defaulting to 32"
            return 32
        }

        Write-Verbose "Detected actual icon size: ${maxSize}x${maxSize}"
        return $maxSize
    } catch {
        Write-Verbose "Could not detect icon size, defaulting to 32"
        return 32
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC0IepkSt/bzeb9
# usVMY6gYBXFwou3R24F3Fj/2TIDzJqCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IE+OYpFcqdY4ZHENZpCy88iKyWLFWvi2rbN0WhvImtn2MA0GCSqGSIb3DQEBAQUA
# BIIBgLqOeH6Ev5wWQqQ6jqfzvYXUB6fJSJ8YYdQ62THGpgqbsm1JgvP6cOgxSbSA
# ck7Cyx1c2qlo/DdfaF+VWCmvlh6+hCxS1rCO8CiNfW3JYtucLbS+3Ufgf2xBPAVM
# a91FkWtUIB9CNh6nVtKQBnf0tAeMcrfnTgQ69OSEaCLtre51hVw2qwcSkL0WSfRJ
# 93cSVGcOjWLwmHySxEtTSYd4H/sAkPQpc0iHIo03SZiS4rorzH8OVXcUYw9n2dKj
# BLUaZQSwo4Av1e1+XvCgXNW01JzNhE+lpvCG9ooPtszCtNh07vdSE94o6hA6PeFI
# EavbSUXnZzgvRVi6E4JpJZgl2bsKbWGN2fyUXxbI319p/O5rmBiaRd8LbQsp1MOo
# Yi4cUJtqFl8NXqAvbzupSKUbvFgd6tZSn1n5QFW3g+Vi3nWAui2FJjVnmoEka/a0
# K4/mWyHIH+0MpdiCJ/hYiY1DmbtuiGmbq/g8E5JqWOQt1l3g3FXAjzkM3GwWKJKV
# Tw0fSKGCFLIwghSuBgorBgEEAYI3AwMBMYIUnjCCFJoGCSqGSIb3DQEHAqCCFIsw
# ghSHAgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFqBgsqhkiG9w0BCRABBKCCAVkEggFV
# MIIBUQIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFlAwQCAQUABCAbXAv/xq6d/pjW
# JLmG65AHlNFrqss4YLHGwiok0lsfBwIGacZiOPDpGBMyMDI2MDQxNTE5NDQxMy4w
...
# AQkEMSIEII3tRCv9YDJc9b2K/WSp9bL5zM2KzWYnVFtPiP4SESQWMIHdBgsqhkiG
...
# 71jXHZnzdbdaJdn9+I9RtQU3Kk+LLlkwDQYJKoZIhvcNAQELBQAEggIAP+9YQZ99
# NCS1m78Zn8/A/skrwm0CShTK+BS+4LwWF5MbroqyU9lfwcyEctF6W8ioZI6dsjGG
# eWvxKvh1ahhHLWMBlWnaLE2QvCRmuPiknPYl1u6d5zDjClNZtsFqPfrIPEIngZtR
# xsMr0MSPDEX4r/yl17T5kJw2+sA0L/FGa2QPhyedOrKt73zM43U0SXCU/jEhnWgt
# CiUdzhBont96gt2uHbZoF6626Ekh/IW69fXG+WT00UaPOHAA8YsIhliKemiM+FN4
# FcqIwBN7TnBaTIpJVzDpfkm2wQQ85FeJ1W+dhiqd3mOor3z6fhGuz80z7Xx7s8RR
# Bwp2p/hExl1az8pnm7m9OjUXoCuPSWiyCPeUTcNZI4HpOSSTPrzXktsbLnGzzIdA
# VQJmxz9uZru1iOqUR9rSeClWMTmTN8QSGfdICnskG0qYlpvPWK6f8w111jm4ZF81
# miDH+X5oYmgcDq9yS1vRyZOqRAWlBDNZqzmndUbN/2Epi7gIexJ3Eol2h0T9W5/U
# f4qIa3UlkDWnv4rE2DFWTJMF3wz4UIztlNBZcliV7Nc02KzIYTAJdYd+M2u76ua9
# um84oQCGfkgqJls5kIRupVSZg7ltw/7TYSGnfVJx21YKuZ1aDoD/hTbT52eEZuMW
# ZmYGoyVAlmLU9Fqmnu8j1wRcb3vVGH910Wo=
