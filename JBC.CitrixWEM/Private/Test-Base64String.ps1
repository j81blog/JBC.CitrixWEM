<#
.SYNOPSIS
    Tests whether a string is a valid Base64-encoded string.

.DESCRIPTION
    Validates a string against Base64 encoding rules: character set, padding, and length.
    Optionally attempts to decode the string to confirm it is decodable.
    Use -PassThru to return the decoded value instead of a boolean.
    Default PassThru output is a UTF-8 decoded string; use -AsByte to return raw bytes.

.PARAMETER InputString
    The string to validate.

.PARAMETER TryDecode
    Attempts to decode the Base64 string to confirm it is decodable.
    Implied when -PassThru is used.

.PARAMETER PassThru
    Returns the decoded value on success instead of $true.
    Default output is a UTF-8 string. Use -AsByte to return a byte array.
    Returns $null on failure instead of $false.

.PARAMETER AsByte
    When used with -PassThru, returns the decoded byte array instead of a string.

.PARAMETER AsString
    When used with -PassThru, explicitly returns the decoded value as a UTF-8 string.
    This is the default PassThru behavior; included for pipeline clarity.

.EXAMPLE
    Test-Base64String -InputString "SGVsbG8gV29ybGQ="
    Returns $true.

.EXAMPLE
    Test-Base64String -InputString "SGVsbG8gV29ybGQ=" -PassThru
    Returns "Hello World" (UTF-8 decoded string).

.EXAMPLE
    Test-Base64String -InputString "SGVsbG8gV29ybGQ=" -PassThru -AsByte
    Returns the decoded byte array.

.EXAMPLE
    "SGVsbG8gV29ybGQ=", "not-base64!" | Test-Base64String -PassThru
    Pipeline usage; returns decoded string or $null per input.

.NOTES
    Function  : Test-Base64String
    Author    : John Billekens
    Copyright : Copyright (c) AppVentiX & John Billekens Consultancy
    Version   : 2026.0306.1230
#>
function Test-Base64String {
    [CmdletBinding(DefaultParameterSetName = 'Boolean')]
    [OutputType([bool], ParameterSetName = 'Boolean')]
    [OutputType([string], ParameterSetName = 'PassThruString')]
    [OutputType([byte[]], ParameterSetName = 'PassThruByte')]
    param (
        [Parameter(
            Mandatory = $true,
        ...
        [Parameter(Mandatory = $true, ParameterSetName = 'PassThruString')]
        [Parameter(Mandatory = $true, ParameterSetName = 'PassThruByte')]
        [switch]$PassThru,

        [Parameter(ParameterSetName = 'PassThruString')]
        [switch]$AsString,

        [Parameter(ParameterSetName = 'PassThruByte')]
        [switch]$AsByte
    )

    process {
        $failResult = if ($PassThru) { $null } else { $false }

        if ([string]::IsNullOrEmpty($InputString)) {
            Write-Verbose "Input is null or empty."
            return $failResult
        }

        if ($InputString.Length % 4 -ne 0) {
            Write-Verbose "Length $($InputString.Length) is not a multiple of 4."
            return $failResult
        }

        if ($InputString -notmatch '^[A-Za-z0-9+/]*={0,2}$') {
            Write-Verbose "String contains invalid Base64 characters."
            return $failResult
        }

        $paddingCount = ($InputString.ToCharArray() | Where-Object { $_ -eq '=' }).Count
        if ($paddingCount -gt 2) {
            Write-Verbose "Excessive padding detected: $($paddingCount) '=' characters."
            return $failResult
        }

        if ($PassThru -or $TryDecode) {
            try {
                $decoded = [System.Convert]::FromBase64String($InputString)
            } catch {
                Write-Verbose "Decode attempt failed: $($_.Exception.Message)"
                return $failResult
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDOjMBGF4hRRLo+
# bS7BJROUw9L6k0lR8E/JMhBvMFvM9aCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
