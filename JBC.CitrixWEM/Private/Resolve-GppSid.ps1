function Resolve-GppSid {
    <#
    .SYNOPSIS
        Resolves a SID to an Active Directory Group or User object.

    .DESCRIPTION
        This helper function attempts to resolve a Security Identifier (SID) to its corresponding
        Active Directory Group or User object. Returns a custom object with SID, Name, and Type
        properties, or null if the SID cannot be resolved.

        Used by GPP functions (Get-GppShortcut, Get-GppPrinterMapping, Get-GppDriveMapping) to
        resolve Item-Level Targeting security groups and users.

    .PARAMETER Sid
        The Security Identifier (SID) to resolve.

    .PARAMETER ItemName
        The name of the GPP item being processed (used for warning messages).

    .OUTPUTS
        PSCustomObject with properties: Sid, Name, Type (group or user)
        Returns $null if the SID cannot be resolved.

    .EXAMPLE
        $resolved = Resolve-GppSid -Sid "S-1-5-21-..." -ItemName "MyApp"

    .NOTES
        Used internally by GPP functions for Item-Level Targeting SID resolution.
    #>
    [CmdletBinding()]
    param(
    ...
    try {
        if ($Group = Get-ADGroup -Identity $Sid -ErrorAction SilentlyContinue) {
            return [PSCustomObject]@{
                Sid  = $Sid
                Name = $Group.Name
                Type = "group"
            }
        } elseif ($User = Get-ADUser -Identity $Sid -ErrorAction SilentlyContinue) {
            return [PSCustomObject]@{
                Sid  = $Sid
                Name = $User.Name
                Type = "user"
            ...
            Write-Warning "Item: `"$ItemName`", Could not resolve SID '$Sid' to an AD Group or User name."
            return $null
        }
    } catch {
        Write-Warning "Item: `"$ItemName`", Could not resolve SID '$Sid' to an AD Group or User name."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCAuh/WvNhUo7Mn
# fvC/74Dz96SDmDc42+1cUlJxGge1+KCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IBLu+WIfRcw1VVEP/GbGa9ISyyOzspRLwpGm2/JouOpZMA0GCSqGSIb3DQEBAQUA
# BIIBgJv+D1ryZ0ZFjx0rlvinBjUzWIc/v7/HAWkk8jLHB20VaUtlAaGQuitXM8eP
# VbjfvuiHCXBA26FPHo6/av6OcitHo060l0WBlgaQl84A3QOeDziY2SUg0xB//rT+
# zCrVMdvIkwZ/6CSyA95Kw0rTcu/59C17Zbc0/Iy0qiajQLYSLHHV6TCDRrVS7aMQ
# +XKB0KTIVgEeckghTT5v7tg34cXW6yfhZOocilnrHEK5MozwOy3fgnWPk0FP6JkK
# NVW1RwY3PFYKnjtN1N59p0gGpNDZ3SSuCojSFiCybDEMai05ZYu/hECO+Iw6hFue
# jf2xMzX2cOpb3x8vmafqVLF/zEQrTcYdnBwWaMEzFbF43CqUAQyeHNJYVWxK+YEz
