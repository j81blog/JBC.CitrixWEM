<#
    .SYNOPSIS
        This function retrieves an Active Directory object by name or LDAP filter.

    .DESCRIPTION
        The Get-AdsiADObject function retrieves an Active Directory object by name or LDAP filter. It searches for the object in the domain of the current user and returns the object's name, sAMAccountName, distinguishedName, objectClass, and other properties.

    .PARAMETER Identity
        Specifies the identity (Name) of the Active Directory object to retrieve.

    .PARAMETER SearchLimit
        Specifies the maximum number of search results to return. Default is 200.

    .PARAMETER SearchBase
        Specifies the search base for the object search. If not specified, the search will be performed in the domain of the current user.

    .PARAMETER LDAPFilter
        Specifies an LDAP filter to narrow down the search results. This parameter is mandatory when using the 'LDAPFilter' parameter set.

    .PARAMETER Credential
        Specifies the credentials to use for the search. If not specified, the search will be performed with the credentials of the current user.

    .PARAMETER Server
        Specifies the domain controller to connect to for the search. If not specified, the search will be performed on the default domain controller.

    .PARAMETER Port
        Specifies the port number to use for the LDAP connection. Default is 389.

    .PARAMETER ObjectType
        Specifies the type of object to search for. Valid values are "*", "user", "group", and "computer". Default is "*".

    .PARAMETER Properties
        Specifies the properties to retrieve for the object. Default is "*".

    .EXAMPLE
        Get-AdsiADObject -Identity "JohnDoe"
        This command retrieves the Active Directory object with the name "JohnDoe".

    .EXAMPLE
        Get-AdsiADObject -LDAPFilter "(|(objectClass=user)(objectClass=group))"
        This command retrieves all Active Directory objects that are either users or groups.

    .NOTES
        Function  : Get-AdsiADObject
...
function Get-AdsiADObject {
    ...
    Write-Verbose "Starting function : Get-AdsiADObject"
    if ($Port -eq 0) { $Port = 389 }
    $adsiADParams = @{
        Port   = $Port
        Server = $Server
    }
    ...
    $adsiADParams.Credential = $Credential
    $adsiADDomain = Get-AdsiADDomain @adsiADParams
    Write-Verbose "Domain            : $($adsiADDomain.DistinguishedName)"
    Write-Verbose "Server            : $($adsiADDomain.DirectoryServer)"
    Write-Verbose "LDAPPath          : $($adsiADDomain.LDAPPath)"

    if ([String]::IsNullOrEmpty($ObjectType) -or $ObjectType -eq "*") {
        $ObjectClass = "(|(objectClass=group)(objectClass=user)(objectClass=computer))"
    } else {
        $ObjectClass = "(objectClass=$ObjectType)"
    }

    if (-Not [String]::IsNullOrEmpty($Identity)) {
        if ($WildcardSearch) {
            $searchIdentityString = "*$Identity*"
        } else {
            $searchIdentityString = $Identity
        }
        $LDAPFilter = "(&$($ObjectClass)(|(Name=$searchIdentityString)(UserPrincipalName=$searchIdentityString)(SamAccountName=$searchIdentityString)(Sn=$searchIdentityString)(GivenName=$searchIdentityString)(ObjectSid=$searchIdentityString)))"
    }
    if ($adsiADDomain.LDAPPath -like "LDAP://*") {
        $LDAPPath = '{0}/' -f $adsiADDomain.LDAPPath.TrimEnd("/")
    } else {
        $LDAPPath = "LDAP://"
    ...
    if ([String]::IsNullOrEmpty($SearchBase)) {
        $LDAPPath += $adsiADDomain.DistinguishedName
    ...
    try {
        if ($null -eq $Credential -or $Credential -eq [Management.Automation.PSCredential]::Empty) {
            ...
            Write-Verbose "Connecting        : Without credential"
        } else {
            ...
            Write-Verbose "Connecting        : With credential"
        }
        ...
        Foreach ($property in $Properties) {
            $searcher.PropertiesToLoad.Add("$($property)".ToLower()) | Out-Null
        }

        # Ensure required properties are always loaded (unless using wildcard)
        $requiredProperties = @("distinguishedname", "name", "objectclass", "samaccountname", "objectsid")
        $hasWildcard = $searcher.PropertiesToLoad -contains "*"

        foreach ($requiredProp in $requiredProperties) {
            if (-not $hasWildcard -and -not ($searcher.PropertiesToLoad -contains $requiredProp)) {
                $searcher.PropertiesToLoad.Add($requiredProp) | Out-Null
                Write-Verbose "Added required property: $requiredProp"
            }
        }

        Write-Verbose "Properties to load: $($searcher.PropertiesToLoad -join ', ')"

        $searcher.PageSize = $SearchLimit

        $result = $searcher.FindAll()
        Write-Verbose "Result count      : $($result.count)"
        ...
        if ($result.count -gt 0) {
            ...
            ForEach ($entry in $result) {
                ...
                ForEach ($propertyName in $entry.Properties.PropertyNames) {
                    $propertyValue = $entry.Properties[$propertyName]

                    # Use centralized conversion function for all property values
                    $entryProperties[$propertyName] = ConvertFrom-ResultPropertyValue -PropertyName $propertyName -PropertyValue $propertyValue
                }

                Write-Output ([PSCustomObject]@{
                        Name              = "$($entryProperties["name"])"
                        sAMAccountName    = "$($entryProperties["samaccountname"])"
                        DistinguishedName = "$($entryProperties["distinguishedname"])"
                        objectClass       = "$($entryProperties["objectclass"])"
                        OtherProperties   = [PSCustomObject]$entryProperties
                    })
            }
        } else {
            if ($PSCmdlet.MyInvocation.BoundParameters.ContainsKey('Identity')) {
                Write-Warning "Object with name '$Identity' not found in Active Directory."
            }
            if ($PSCmdlet.MyInvocation.BoundParameters.ContainsKey('LDAPFilter')) {
                Write-Warning "Object with LDAP filter '$LDAPFilter' not found in Active Directory."
            }
            Write-Output (@())
        }
    } finally {
    ...
    Write-Verbose "Ending function   : Get-AdsiADObject"
}

# SIG # Begin signature block
# MII6BgYJKoZIhvcNAQcCoII59zCCOfMCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCtevVNEzXhKlSe
# cTskwIYocGvBkZpp0M+Ro9Lgw301fKCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IPxZGPhKX0AQ7PuIwqtppSKnvL6Q1csJQZyvlrrlCvycMA0GCSqGSIb3DQEBAQUA
# BIIBgEOHEmlTuoJdj2R4Ph9Sd57Kfv75n1pAbgrI0hfALDOmUfUkOkVhlcHwwNR6
# qklusYPkwC8I3qpeER8Ca3xuVFGPolAeFHBL2SLSsycWl9r6P7pDf3iTiM/zB4p4
# cHI2giPPlB/BE4h4XjcJZd7seDxFtznHF6Nh9bW+rVcq4i5o0dE0uDwdcLGnlmLB
