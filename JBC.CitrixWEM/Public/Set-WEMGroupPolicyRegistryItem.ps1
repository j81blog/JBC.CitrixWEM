function Set-WEMGroupPolicyRegistryItem {
    <#
    .SYNOPSIS
        Modifies an existing registry operation within a WEM Group Policy Object in memory.
    .DESCRIPTION
        This function locates a registry operation by Key, Value, and Scope, then updates
        the specified properties. It modifies the object in memory and returns the updated
        GPO -- no API call is made.
        Pipe the result to Set-WEMGroupPolicyObject to persist the changes.
    .PARAMETER InputObject
        The Group Policy Object to modify. Typically from Get-WEMGroupPolicyObject -IncludeRegOperations.
    .PARAMETER Key
        The registry key path that identifies the operation to modify.
    .PARAMETER Value
        The registry value name that identifies the operation to modify.
    .PARAMETER Scope
        The scope that identifies the operation to modify. Valid values: Machine, User.
    .PARAMETER NewKey
        The new registry key path to set on the matched operation.
    .PARAMETER NewValueName
        The new registry value name to set on the matched operation.
    .PARAMETER NewScope
        The new scope to set on the matched operation. Valid values: Machine, User.
    .PARAMETER NewAction
        The new action to set on the matched operation.
    .PARAMETER NewType
        The new registry type to set on the matched operation.
    .PARAMETER NewData
        The new registry data to set on the matched operation.
    .EXAMPLE
        PS C:\> Get-WEMGroupPolicyObject -IncludeRegOperations | Where-Object Name -eq "My GPO" |
        PS C:\>     Set-WEMGroupPolicyRegistryItem -Key "Software\Company" -Value "Setting" -Scope Machine -NewData "UpdatedValue" |
        PS C:\>     Set-WEMGroupPolicyObject

        Updates the registry data of the specified operation in "My GPO" and saves it.
    .EXAMPLE
        PS C:\> Get-WEMGroupPolicyObject -IncludeRegOperations | Where-Object Name -eq "My GPO" |
        PS C:\>     Set-WEMGroupPolicyRegistryItem -Key "Software\Company" -Value "Setting" -Scope User -NewScope Machine -NewData "42" -NewType REG_DWORD |
        PS C:\>     Set-WEMGroupPolicyObject

        Moves a User-scope operation to Machine scope and changes its type and data.
    .NOTES
        Version:        1.0
    ...
    param(
        ...
        # Identifiers -- used to locate the operation
    ...
    process {
        $MatchedOps = @($InputObject.registryOperations | Where-Object {
                $_.key -eq $Key -and $_.value -eq $Value -and $_.scope -eq $Scope
            })

        if ($MatchedOps.Count -eq 0) {
            Write-Warning "No registry operation found matching Key '$($Key)', Value '$($Value)', Scope '$($Scope)'. No changes made."
            Write-Output $InputObject
            return
        }

        foreach ($Op in $MatchedOps) {
            if ($PSBoundParameters.ContainsKey('NewKey')) { $Op.key = $NewKey }
            if ($PSBoundParameters.ContainsKey('NewValue')) { $Op.value = $NewValue }
            if ($PSBoundParameters.ContainsKey('NewScope')) { $Op.scope = $NewScope }
            if ($PSBoundParameters.ContainsKey('NewAction')) { $Op.action = $NewAction }
            if ($PSBoundParameters.ContainsKey('NewType')) { $Op.regType = $NewType }
            if ($PSBoundParameters.ContainsKey('NewData')) {
                $ResolvedType = if ($PSBoundParameters.ContainsKey('NewType')) { $NewType } else { $Op.regType }
                $Op.regData = if ($ResolvedType -eq "REG_MULTI_SZ") { [array]$NewData } else { $NewData }
            }
        }

        Write-Verbose "Updated $($MatchedOps.Count) operation(s) matching Key '$($Key)\$($Value)' (Scope: $($Scope))."
        Write-Output $InputObject
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCsfLB1ulNffo/n
# WnFwej4BrVz2zqG6b16AXTeCDwh41KCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
