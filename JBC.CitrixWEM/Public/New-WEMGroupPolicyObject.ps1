function New-WEMGroupPolicyObject {
    <#
    .SYNOPSIS
        Creates a new registry-based Group Policy Object in a WEM Configuration Set.
    .DESCRIPTION
        This function adds a new Group Policy Object containing registry operations.
        If -SiteId is not specified, it uses the active Configuration Set defined by
        Set-WEMActiveConfigurationSite.

        Use New-WEMGroupPolicyRegistryItem to build each entry for the -RegistryOperations parameter.
    .PARAMETER Name
        The unique name for the Group Policy Object.
    .PARAMETER Description
        An optional description for the Group Policy Object.
    .PARAMETER RegistryOperations
        An array of registry operation objects created with New-WEMGroupPolicyRegistryItem.
    .PARAMETER SiteId
        The ID of the WEM Configuration Set. Defaults to the active site.
    .PARAMETER PassThru
        If specified, the command returns the newly created Group Policy Object.
    .EXAMPLE
        PS C:\> $ops = @(
        PS C:\>     New-WEMGroupPolicyRegistryItem -Action SetValue -Scope Machine -Key "Software\Company" -ValueName "Setting" -RegType REG_SZ -RegData "Value"
        PS C:\>     New-WEMGroupPolicyRegistryItem -Action CreateKey -Scope User -Key "Software\Company\SubKey"
        PS C:\> )
        PS C:\> New-WEMGroupPolicyObject -Name "My GPO" -Description "Test" -RegistryOperations $ops

        Creates a new Group Policy Object with two registry operations.
    .NOTES
        Version:        1.0
    ...
    try {
        ...
        if ($PSCmdlet.ShouldProcess($Name, "Create WEM Group Policy Object in Site ID '$($ResolvedSiteId)'")) {
            # Ensure REG_MULTI_SZ entries are arrays (PS may unwrap single-element arrays to strings)
            ...
            if ($PassThru.IsPresent) {
                Write-Verbose "PassThru specified, retrieving newly created Group Policy Object..."
                $Result = Get-WEMGroupPolicyObject -SiteId $ResolvedSiteId | Where-Object { $_.Name -ieq $Name }
            }

            Write-Output ($Result | Expand-WEMResult -ErrorAction SilentlyContinue)
        }
    } catch {
        Write-Error "Failed to create WEM Group Policy Object '$($Name)': $($_.Exception.Message)"
        return $null
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAnZBlJ6ThZmdEB
# oThb21fVUE/AOEuxBe9iUm7Q/xg12aCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IDiNSEukivBt0PkF1+jUfO+XX4gugj7HZAx6WehpEEGGMA0GCSqGSIb3DQEBAQUA
# BIIBgDK6umJFqOo2nmQueqt7YHq/LhPq5TZKpqxwjvyrvRN8GQjCOJytCzlf5Rgm
# gNrMPvJ66t3+SM6Pd1QQHcKC67YNjh42PHuQIaYG6yMrBMk5b1j4Wd6njskmNYMF
# ilZCIVZlh2F4ncQCf7TMlz6goPqtmgyfZ8rPiUzQ7UphpHJxh7wYsd/a8UkGDYm0
# L/rT+gpfzQaFUlIh6kn04T7qf9q3nlPj3thHEghxoFuy053lKEGL6lIh6vbkF41x
