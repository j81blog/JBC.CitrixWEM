function Import-IvantiWCEnvironmentVariable {
    <#
    .SYNOPSIS
        Imports Ivanti Workspace Control environment variables into Citrix WEM.
    .DESCRIPTION
        Reads environment variable configurations from an Ivanti Workspace Control Building Block
        XML file using Get-IvantiWCEnvironmentVariable, creates a WEM Environment Variable action
        for each entry via New-WEMEnvironmentVariable, and assigns it to the appropriate AD targets
        via New-WEMEnvironmentVariableAssignment.

        Disabled environment variables are skipped. AD users and groups are resolved from the WEM
        active domain; if a group cannot be found it is skipped with a warning. Assignment targets
        that do not yet exist in WEM are created automatically.
    .PARAMETER XmlFilePath
        Path to the Ivanti Workspace Control Building Block XML file.
    .PARAMETER DomainFqdn
        Domain FQDN to use when resolving AD objects. Defaults to the current user's DNS domain
        ($Env:USERDNSDOMAIN).
    .PARAMETER SiteId
    ...
    .PARAMETER GUI
        If specified, opens a grid view to allow selection of which environment variables to import.
    .EXAMPLE
        Import-IvantiWCEnvironmentVariable -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1

        Imports all enabled environment variables from the building block into Configuration Set 1.
    .EXAMPLE
        Import-IvantiWCEnvironmentVariable -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1 -GUI

        Opens a selection grid before importing, letting you choose which environment variables to process.
    .NOTES
        Function  : Import-IvantiWCEnvironmentVariable
    ...
    if ($ActiveDomain = Get-WEMActiveDomain) {
        ...
        foreach ( $Item in $itemsToProcess ) {
            ...
            try {
                ...
                try {
                    $WEMEnvironmentVariable = New-WEMEnvironmentVariable @Params -SiteId $ResolvedSiteId -PassThru -ErrorAction Stop
                ...
                foreach ($WEMAssignment in $Item.WEMAssignments) {
                    ...
                    $WEMAssignmentTarget = Get-WEMAssignmentTarget -SiteId $ResolvedSiteId | Where-Object { $_.name -ieq $WEMAssignmentTargetGroupName -or $_.name -ieq $WEMAssignment.Name -or $_.sid -ieq $WEMAssignment.Sid }
                    ...
                    $WEMEnvironmentVariableAssignment = New-WEMEnvironmentVariableAssignment -SiteId $ResolvedSiteId -Target $WEMAssignmentTarget -EnvironmentVariable $WEMEnvironmentVariable -PassThru @WEMAssignmentParams
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDT0B4eiPZHydi8
# vz4MzXmfn2y8E4o3StjgwcszI6qyfqCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IPRN07Vwzcww5gq4gVDWiGOC9XuUqTn8vE2Xjdfni3HZMA0GCSqGSIb3DQEBAQUA
# BIIBgLIm954y9l6zRcn4RSuBZhG0kR2atyddZbk26LrWU5mYoAmKWwi4Mio0Djoh
# 1XJeI5QFOGvVeBb7pbKXjwIge+QX4Yp+ZarepjBUJJdG3d1OtWsw8kIzGOhG0n8k
