function Import-IvantiWCPrinterMapping {
    <#
    .SYNOPSIS
        Imports Ivanti Workspace Control printer mappings into Citrix WEM.
    .DESCRIPTION
        Reads printer mapping configurations from an Ivanti Workspace Control Building Block XML
        file using Get-IvantiWCPrinterMapping, creates a WEM Printer action for each entry via
        New-WEMPrinter, and assigns it to the appropriate AD targets via New-WEMPrinterAssignment.

        Disabled printer mappings are skipped. AD users and groups are resolved from the WEM active
        domain; if a group cannot be found it is skipped with a warning. Assignment targets that do
        not yet exist in WEM are created automatically.
    ...
    .PARAMETER GUI
        If specified, opens a grid view to allow selection of which printer mappings to import.
    .EXAMPLE
        Import-IvantiWCPrinterMapping -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1

        Imports all enabled printer mappings from the building block into Configuration Set 1.
    .EXAMPLE
        Import-IvantiWCPrinterMapping -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1 -GUI

        Opens a selection grid before importing, letting you choose which printer mappings to process.
    .NOTES
        Function  : Import-IvantiWCPrinterMapping
    ...
    if ($ActiveDomain = Get-WEMActiveDomain) {
        ...
        foreach ( $Item in $itemsToProcess ) {
            ...
            try {
                ...
                try {
                    $WEMPrinter = New-WEMPrinter @Params -SiteId $ResolvedSiteId -PassThru -ErrorAction Stop
                ...
                foreach ($WEMAssignment in $Item.WEMAssignments) {
                    ...
                    $WEMPrinterAssignment = New-WEMPrinterAssignment -SiteId $ResolvedSiteId -Target $WEMAssignmentTarget -Printer $WEMPrinter -PassThru @WEMAssignmentParams
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA22QnGGDE6GeaB
# nrtOyoToB6VjhC1G5UxoO2T1nQIBAqCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IHa691QsHxRHD9Nv7qF8BohTIAjCFgXQLFr9zPAae+IjMA0GCSqGSIb3DQEBAQUA
# BIIBgDRvWowjyeRbjikwdFTv97w0OPBKHMslW4lt7T01PNAYnUAzs62uJJjv/gZ8
# jJ23TT2yV9HYMN1bkYdJ08hf03wTkZe6eMOA4UUuzNQuOaYf4HQcIhqt7yEMh/Lp
# YTggWks/fd9pg252LChWM7zkuUDMAM+bCvmypjSeAM7VoBm7ZDS1Cp7zFzDUWnWm
# pzzyUOc6EmvRJl3RuUDEWI3xcQR97LDdWW4trNjgNAR+Or38zU9l308onvLcoX6A
# 2pUn9CYXp24BTMZxSXV+EoHWUARPgYGMYFPbeVljwP610AsAjA3k/ooSfMZWANaw
# OCs+tkc/0U/2wQrbqbNdYcJBs/C3NWhoGDsH4tc8Hz7nSGV3jb/RXOsqLYrThpCj
# JnUpKt4AivLgbDssRnMcjyzLYbKSREjR2uJryVKcHBdmd7S/SOwys8hLD7rZrdZI
# kBcXfY1Xj+YvwdmupFn7bOPg4CeW5td6EoEGBsuOnARe4G20yswvYkTPi5y6Lb54
# efEBl6GCFLIwghSuBgorBgEEAYI3AwMBMYIUnjCCFJoGCSqGSIb3DQEHAqCCFIsw
# ghSHAgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFqBgsqhkiG9w0BCRABBKCCAVkEggFV
# MIIBUQIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFlAwQCAQUABCBDYfBkhIf0Ew0t
# YKqhjx53SpxZyXeubUUWvlnLmMZFigIGacZiOPEcGBMyMDI2MDQxNTE5NDcwOC42
# MDJaMASAAgH0oIHppIHmMIHjMQswCQYDVQQGEwJVUzETMBEGA1UECBMKV2FzaGlu
...
# AQkEMSIEIDM6S2gru8J23+xHFXhHNjmMhI4Xj+50HmRYvxYmdLHBMIHdBgsqhkiG
...
# 71jXHZnzdbdaJdn9+I9RtQU3Kk+LLlkwDQYJKoZIhvcNAQELBQAEggIAuPVZRX4j
# DHYfEuVm2cnVywuzuaqKZ0RYbDrzTq3jwmwCsEYHZMacGWlPG0UdO/W76m1245wt
# 3Th4Ra235XLLYIMv+idQFsPoL/4dwGxaA/d9QWyy/Ds+cWZ/P/pLDflCyGX1bqAJ
# uaKNiC0/yvTiis3qBLqs/NycbXYzqRZB75ceXbu2/nQ8ccWrgJXVjWfONgRLi1I3
# w9r4nGcyEE5FSLqMnZ3XSCYd3Maqq/ItiSHBcghWRkYyrlnnU6ODUyXyv10lczX1
# IOOUf0uXVRifQRPdbFTo1q5jKXaoRHzE+8bd+svKN8d76pOjy1AsYMduOewr8Bw6
# 9nI+W5iiK3z55EmfPMwB6MXJlgvcH7tz0R3joyjnP3duf8fu1fl9LwadfQvRQMV+
# R/s+qJ/82GAJMACMaJoXpoSnxaXpa3NJrS5LihE6kL2T2dpiWVXQaDocaqIcwFKG
# Ag5BMzAMInhmD9m2vpsLbLrMmomwZzpWYR7glnhhEtr3svq9eLSyBQzy+Y/0pKnn
# I22aJLFMruFWaOcY2rRZGOBHdKcJXjfr4h7tQHWOMff5B9jTrdJrLswbwoOmarLF
# BmV5Oq4PPWQm4WGxQskiW3UP6AV2ENbULMiFdrHa+lCMQ8HlshrzqhO9V+YlAtAC
# oGL0xm94zZGbkFpS7Upno9B03AovBL3wfUo=
