function Get-ShortcutDetails {
    <#
    .SYNOPSIS
        Retrieves detailed information from Windows shortcut (.lnk) files.

    .DESCRIPTION
        Get-ShortcutDetails reads one or more Windows shortcut (.lnk) files and returns a
        structured object for each shortcut containing its target path, arguments, description,
        icon details, window style, hotkey, and working directory.

        Use the -filename parameter to inspect a single shortcut, or the -Path parameter to
        process all shortcuts in a directory (optionally including subdirectories with -Recurse).

        When -ExtractIcon is specified, the icon referenced by each shortcut is extracted and
        returned as a Base64-encoded string via the IconStream property.

    .PARAMETER filename
        Full path to a single .lnk file to inspect.

    .PARAMETER Path
        Path to a directory. All .lnk files found directly in that directory are processed.
        Combine with -Recurse to include subdirectories.

    .PARAMETER Recurse
        When used with -Path, searches subdirectories recursively for .lnk files.

    .PARAMETER ExtractIcon
        Output path prefix or identifier used when extracting icons. When supplied, the icon
        referenced by each shortcut is exported as a 32x32 Base64-encoded image and included
        in the IconStream property of the output object.

    .OUTPUTS
        PSCustomObject with the following properties:
          ShortcutFilename   - File name of the shortcut (without extension)
          ShortcutFullName   - Full path to the .lnk file
          ShortcutPath       - Directory containing the .lnk file
          TargetName         - File name portion of the target executable or file
          Description        - Shortcut description (comment)
          TargetPath         - Full path to the target executable or file
          TargetArguments    - Command-line arguments passed to the target
          IconStream         - Base64-encoded icon image (only when -ExtractIcon is used)
          IconFile           - Path to the file from which the icon is sourced
          IconIndex          - Index of the icon within the icon file
          WindowStyle        - Numeric window style code (1 = Normal, 3 = Maximized, 7 = Minimized)
          WindowStyleText    - Human-readable window style label
          Hotkey             - Keyboard shortcut assigned to the shortcut
          WorkingDirectory   - Working directory set on the shortcut

    .EXAMPLE
        Get-ShortcutDetails -filename "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Notepad.lnk"

        Returns shortcut details for a single .lnk file.

    .EXAMPLE
        Get-ShortcutDetails -Path "C:\ProgramData\Microsoft\Windows\Start Menu\Programs" -Recurse

        Returns shortcut details for all .lnk files found in the Start Menu programs folder and
        all its subdirectories.

    .EXAMPLE
        Get-ShortcutDetails -Path "C:\Shortcuts" -ExtractIcon "icons" | Select-Object ShortcutFilename, IconStream

        Returns shortcut details including Base64-encoded icons for all shortcuts in C:\Shortcuts.
    .NOTES
        Function  : Get-ShortcutDetails
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCyFWZypeluI6PX
# nF9SiuT0HrA7QfyBn990gT1ZFoSrs6CCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IDyen1akpufJA8RQa3HILyc21wSNjMVJBl86+ZQqqcpbMA0GCSqGSIb3DQEBAQUA
# BIIBgGyjMQGinPrlxnyTKBnAkcZS37vTHrl3GFA2FdC3OCAcvut0P4SlDMR1Z1O/
# jEpPi+XQv4+mZKhUctr3GUlu0T+eKZBl/HQjJUP2t35R73xckW/AsqY3le1dO2xW
# yE1Gu3x0CFYW2yPCSZX//a5eMJcpk+Z0jKrvyzzx5DcXxSCEUgBY1e+pTZRjBwoW
