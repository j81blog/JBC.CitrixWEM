function Get-GppPrinterMapping {
    <#
    .SYNOPSIS
        Retrieves Group Policy Preferences printer mappings from specified GPOs.

    .DESCRIPTION
        The Get-GppPrinterMapping function extracts shared printer mapping information from
        ...
        Active Directory security groups and users from Item-Level Targeting filters.

        The function can optionally convert printer paths to FQDN format and filter out
        printer mappings with Delete actions.
    ...
    .PARAMETER Domainname
        The domain FQDN to use when converting printer paths to FQDN format.
        Required when using -ConvertPathToFQDN.

    .PARAMETER ConvertPathToFQDN
        Converts printer server names to fully qualified domain names.
        Requires the -Domainname parameter.

    .PARAMETER SkipDeleteAction
        Skips printer mappings that have a Delete action configured in the GPO.
    ...
    .OUTPUTS
        ...
        Returns custom objects with printer mapping details, or JSON string if -AsJson is specified.

    .EXAMPLE
        Get-GppPrinterMapping -GpoName "Printers Kantoor Policy"

        Retrieves all GPP printer mappings from the specified GPO.

    .EXAMPLE
        Get-GppPrinterMapping -GpoName "Printers Kantoor Policy" -ConvertPathToFQDN -Domainname "contoso.com"

        Retrieves printer mappings and converts printer server names to FQDN format.

    .EXAMPLE
        "GPO1", "GPO2" | Get-GppPrinterMapping -SkipDeleteAction -AsJson

        Processes multiple GPOs via pipeline, excludes Delete actions, and returns JSON output.

    .EXAMPLE
        Get-GppPrinterMapping -GpoName "Printers Policy" -JsonFilePath "C:\Export\printers.json"

        Exports all printer mappings from the GPO to a JSON file.
    ...
    process {
        ...
        foreach ($Name in $GpoName) {
            ...
            try {
                ...
                if ($null -ne $PrinterNodes -and $PrinterNodes.Count -gt 0) {
                    Write-Verbose "Found $($PrinterNodes.Count) GPP Printer Mappings in GPO '$Name'."
                    $PrinterCounter = 0
                    foreach ($Printer in $PrinterNodes) {
                        Write-Progress -Activity "Processing Printer Mappings" -Status "Processing printer $($PrinterCounter + 1) of $($PrinterNodes.Count) in GPO '$Name'" -CurrentOperation "Printer: $($Printer.name)" -PercentComplete ((($PrinterCounter + 1) / $PrinterNodes.Count) * 100) -Id 1 -ParentId 0
                        Write-Verbose "Processing printer: '$($Printer.name)'"

                        if ($SkipDeleteAction -and $Printer.Properties.action -ieq "D") {
                            Write-Verbose "Skipping printer mapping: '$($Printer.name)' with Delete action as per parameter."
                            $PrinterCounter++
                            continue
                        }

                        # Initialize variables
                        $Enabled = $Printer.disabled -ne "1"
                        $IsDefault = $Printer.Properties.default -eq "1"
                        $TargetedGroup = @()

                        # Collect filter nodes
                        $FilterGroupNodes = @($Printer.Filters.FilterGroup)
                        if ($null -ne $Printer.Filters.FilterCollection) {
                            $FilterGroupNodes += $Printer.Filters.FilterCollection.FilterGroup
                        }
                        $FilterUserNodes = @($Printer.Filters.FilterUser)
                        if ($null -ne $Printer.Filters.FilterCollection) {
                            $FilterUserNodes += $Printer.Filters.FilterCollection.FilterUser
                        }

                        # Resolve Group SIDs using helper function
                        if ($null -ne $FilterGroupNodes) {
                            $GroupSids = $FilterGroupNodes | Where-Object { $_.sid } | Select-Object -ExpandProperty sid
                            foreach ($Sid in $GroupSids) {
                                $ResolvedObject = Resolve-GppSid -Sid $Sid -ItemName $Printer.name
                                if ($null -ne $ResolvedObject -and $ResolvedObject.Type -eq "group") {
                                    $TargetedGroup += $ResolvedObject
                                }
                            }
                        }

                        # Resolve User SIDs using helper function
                        if ($null -ne $FilterUserNodes) {
                            $UserSids = $FilterUserNodes | Where-Object { $_.sid } | Select-Object -ExpandProperty sid
                        ...
                        $Action = $ActionMap[$Printer.Properties.action]
                        if (-not $Action) {
                            $Action = $Printer.Properties.action
                        }
                        try {
                            if ($ConvertPathToFQDN -and -not [String]::IsNullOrEmpty($Domainname)) {
                                $PrinterName, $path = $Printer.Properties.path.TrimStart('\\').Split('\', 2) | ForEach-Object { if ($_) { $_ } }
                                if ($PrinterName -like "*.*.*") {
                                    $FQDN = $PrinterName
                                } else {
                                    $FQDN = "$($PrinterName).$($Domainname)"
                                }
                                $PrinterPath = "\\$($FQDN)\$($path)"
                            } else {
                        ...
                        $Output = [PSCustomObject]@{
                            ...
                            WEMPrinterParams    = [PSCustomObject]@{
                                ...
                                Description = $Printer.Properties.comment
                                Enabled     = $Enabled
                            }
                            Enabled             = $Enabled
                        }
                    ...
                    Write-Progress -Activity "Processing Printer Mappings" -Status "Processed $PrinterCounter printers in GPO '$Name'" -Completed -Id 1 -ParentId 0
                } else {
        ...
        if ($JsonFilePath -and $ResultOutput.Count -gt 0) {
            Write-Verbose "Exporting $($ResultOutput.Count) GPP Printer Mappings to JSON file at path: $JsonFilePath"
            $ResultOutput | ConvertTo-Json -Depth 5 | Set-Content -Path $JsonFilePath -Force
        } elseif ($AsJson -and $ResultOutput.Count -gt 0) {
            Write-Verbose "Converting $($ResultOutput.Count) GPP Printer Mappings output to JSON."
            Write-Output ($ResultOutput | ConvertTo-Json -Depth 5)
        } elseif ($ResultOutput.Count -gt 0) {
            Write-Output $ResultOutput
        } else {
            Write-Output "No GPP Printer Mappings found."
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDm78HmujrSLZwd
# Dd/tFFvqG9WpysdwOWifdrRS8WetEaCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IECvVD4mxwTfqPZECRHPHXl6qDxeFSxDxb+BD+QavyIPMA0GCSqGSIb3DQEBAQUA
# BIIBgNmF99CgX8i6Pr7h9JHWcF3jR2//Dv7s+tbeJuivF6l7NBItNtUUocEmdc5p
# pjW73Xnl4YekZFzIcwEALG82pMOyqtmS8x510YYPfGslqY4RU5P99dCxhaI+9tKn
