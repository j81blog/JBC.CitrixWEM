function New-WEMGroupPolicyObjectAssignment {
    <#
    .SYNOPSIS
        Assigns a WEM Group Policy Object to a target.
    .DESCRIPTION
        This function creates a new assignment for a WEM Group Policy Object, linking a target
        (user/group) to a Group Policy Object with an optional filter rule. If -SiteId is not
        specified, it uses the active Configuration Set defined by Set-WEMActiveConfigurationSite.
    .PARAMETER Target
        The assignment target object (e.g., from Get-WEMADGroup or Get-WEMADUser) to which
        the Group Policy Object will be assigned.
    .PARAMETER GroupPolicyObject
        The Group Policy Object (from Get-WEMGroupPolicyObject) to assign.
    ...
    .PARAMETER Priority
        The assignment priority. Defaults to 50.
    .PARAMETER SiteId
    ...
    .EXAMPLE
        PS C:\> $Gpo = Get-WEMGroupPolicyObject | Where-Object { $_.Name -eq "My GPO" }
        PS C:\> $Group = Get-WEMADGroup -Filter "All Users"
        PS C:\> New-WEMGroupPolicyObjectAssignment -Target $Group -GroupPolicyObject $Gpo

        Assigns "My GPO" to the "All Users" group with default priority.
    .NOTES
        Version:        1.0
    ...
    param(
        ...
        [PSCustomObject]$GroupPolicyObject,
    ...
    try {
        ...
        if (-not $GroupPolicyObject.PSObject.Properties['id']) { throw "The provided -GroupPolicyObject object does not have an 'id' property." }
        ...
        $TargetDescription = "Assign Group Policy Object '$($GroupPolicyObject.Name)' (ID: $($GroupPolicyObject.id)) to Target '$($Target.Name)' (ID: $($Target.id))"
        if ($PSCmdlet.ShouldProcess($TargetDescription, "Create Assignment")) {
            $Body = @{
                siteId     = $ResolvedSiteId
                resourceId = $GroupPolicyObject.id
                targetId   = $Target.id
                filterId   = $ResolvedFilterId
                priority   = $Priority
            }

            $UriPath = "services/wem/groupPolicyAssignment"
            ...
            if ($PassThru.IsPresent) {
                ...
                $Result = Get-WEMGroupPolicyAssignment -SiteId $ResolvedSiteId | Where-Object { $_.resourceId -eq $GroupPolicyObject.id -and $_.targetId -eq $Target.id -and $_.filterId -eq $ResolvedFilterId }
            }

            Write-Output ($Result | Expand-WEMResult)
        }
    } catch {
        Write-Error "Failed to create WEM Group Policy Object Assignment: $($_.Exception.Message)"
        return $null
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDgKOXpIZCr9wZh
# rXaPOaWBsu25z3EvbStCGWJBHFpKnKCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IABSFiQwvrJkXVGmBiCLILrppDVVkngkdTEmxf7DEgRLMA0GCSqGSIb3DQEBAQUA
# BIIBgEQSEC+Fo9KxU6Cxuhcy6GarC2PTiK/cRK2fEByK31SZBzs8xlNJdSWmDYrE
# 43Pve6LieUyyALUk/F23O9HNB44AeiMWAaw5SgtaDgs+2k8UXAoQeVNT6QyhHUb0
# xOHGNfYAZqRwaLFmAXyqS6fY17SMJBa4MNzyTzWVExPwX3X6QynD6B1k3KykA5n5
