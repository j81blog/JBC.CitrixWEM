function Import-IvantiWCRegistry {
    <#
    .SYNOPSIS
        Imports Ivanti Workspace Control registry sets into Citrix WEM as Group Policy Objects.
    .DESCRIPTION
        Reads registry sets from an Ivanti Workspace Control Building Block XML file using
        Get-IvantiWCRegistry, creates a WEM Group Policy Object for each registry set via
        New-WEMGroupPolicyObject, and assigns it to the appropriate AD targets via
        New-WEMGroupPolicyObjectAssignment.

        Disabled registry sets are skipped. Registry sets with no registry operations are skipped
    ...
    .PARAMETER GUI
        If specified, opens a grid view to allow selection of which registry sets to import.
    .EXAMPLE
        Import-IvantiWCRegistry -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1
    .EXAMPLE
        Import-IvantiWCRegistry -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1 -GUI
    .NOTES
        Function  : Import-IvantiWCRegistry
    ...
    if ($ActiveDomain = Get-WEMActiveDomain) {
        ...
        $itemsToProcess = @(Get-IvantiWCRegistry -Path $XmlFilePath -ExportFor WEM)

        if ($GUI.IsPresent) {
            $itemsToProcess = @(
                $itemsToProcess | Select-ObjectFromGrid -Title "Select Registry Sets to import from Ivanti IWC" `
                    -ColumnsToShow @('Name', 'Enabled', 'Description')
            )
        }

        if (-not $itemsToProcess -or $itemsToProcess.Count -eq 0) {
            Write-Warning "No registry sets found to import."
            return
        }

        Write-Host "Found $($itemsToProcess.Count) registry set(s) to process"
        ...
        foreach ($Item in $itemsToProcess) {
            ...
            if (-not $Item.Enabled) {
                Write-Host "Skipping registry set '$($Item.Name)' - not enabled" -ForegroundColor Yellow
                continue
            }

            if (-not $Item.WEMParams -or $Item.WEMParams.RegistryOperations.Count -eq 0) {
                Write-Warning "Skipping registry set '$($Item.Name)' - no registry operations found"
                continue
            }
            ...
            try {
                ...
                $Item | Add-Member -MemberType NoteProperty -Name "Success" -Value $false -Force
                Write-Host "Error processing registry set '$($Item.Name)': $(Get-ExceptionDetails -ErrorRecord $_ -AsPlainText)" -ForegroundColor Red
                continue
            }
        }
    } else {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBI8PYpxDRMBHh2
# 8JTHnEMIZuqYjptBj5VTHGL4RSGIgKCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IMqYcwfmL9KZblaMpn4XPzX91nKDBu9+XKGHihT8btrVMA0GCSqGSIb3DQEBAQUA
# BIIBgLsppmZhSN/fLO2UrHWdtPP02UOJOYnOWpYyEw3CxFHFeHOT/fava/3Vvhlj
# lz/IitU6t2bP2bRcLf8lOUKyAUqkZbbT3wnEDK4Cd1vaJkO/5LqXytQtBEidzogs
# m/VHjRn4NUg/4F1pgA1PL651fJCycegrXk5ZRZkDb+TOhVLyHJiTd2iC4JHrbjjR
