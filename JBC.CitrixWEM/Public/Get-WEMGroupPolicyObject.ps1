function Get-WEMGroupPolicyObject {
    <#
    .SYNOPSIS
        Retrieves registry-based Group Policy Objects from a WEM Configuration Set.
    .DESCRIPTION
        This function gets a list of all configured Group Policy Objects from a specified
        WEM Configuration Set (Site).
    .PARAMETER SiteId
        The ID of the WEM Configuration Set (Site) to query.
    .PARAMETER IncludeAssignmentCount
        If specified, the result will include the count of assignments for each object.
    .PARAMETER IncludeRegOperationCount
        If specified, the result will include the count of registry operations for each object.
    .PARAMETER IncludeRegOperations
        If specified, the result will include the full registry operations array for each object.
    .EXAMPLE
        PS C:\> Get-WEMGroupPolicyObject -SiteId 7

        Retrieves all Group Policy Objects for the Configuration Set with ID 7.
    .EXAMPLE
        PS C:\> Get-WEMGroupPolicyObject -SiteId 7 -IncludeAssignmentCount -IncludeRegOperations

        Retrieves all Group Policy Objects including assignment counts and the full registry operations.
    .NOTES
        Version:        1.0
    ...
    param(
        ...
        [Parameter(Mandatory = $false)]
        [switch]$IncludeRegOperationCount,

        [Parameter(Mandatory = $false)]
        [switch]$IncludeRegOperations
    )
    ...
    try {
        ...
        $UriPath = "services/wem/groupPolicyObject?siteId=$($ResolvedSiteId)"
        $UriPath += "&isUpmUserLevelSettingsOnly=false"
        $UriPath += "&isUpmFolderRedirectionSettingsOnly=false"
        if ($IncludeAssignmentCount.IsPresent) {
            $UriPath += "&getAssignmentCount=true"
        }
        if ($IncludeRegOperationCount.IsPresent) {
            $UriPath += "&getRegOperationCount=true"
        }
        if ($IncludeRegOperations.IsPresent) {
            $UriPath += "&includeRegOperation=true"
        }
        ...
        Write-Error "Failed to retrieve WEM Group Policy Objects for Site ID '$($ResolvedSiteId)': $($_.Exception.Message)"
        return $null
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBQ9zHHdmZPCNlW
# Rc6uwZhHHn7f4ijHKtiaNVhiwAbh4aCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# III11yI2rBoicsHolDwe2PrihfAes3BJTFAurA9oKAQUMA0GCSqGSIb3DQEBAQUA
# BIIBgGs2G1Oj3wA2vOfmn1uS+FfzNmiseLLlj7DntcetulKu+R7uQpKz2pLsbbIf
# QEUIEY1fULXgTT3qPTnJCvXRZrpd9LQh4NQgAKoguRi0h+A9CPKa/iNr1kx+9dqE
# oh8XRRfso8W8mDgH0LM94CYLu4P8CnLcocLxhVM2sVEJWGzRk2dNTHXnNPqC1rGd
# 8jY00gbu2z1YxCvCRqOZGOo+TjCsiuuV1DAOvLunUFsJ8MVQFUSdlx0Ad8hCkcNm
