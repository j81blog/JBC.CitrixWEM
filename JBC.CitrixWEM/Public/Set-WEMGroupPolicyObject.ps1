function Set-WEMGroupPolicyObject {
    <#
    .SYNOPSIS
        Updates an existing WEM Group Policy Object.
    .DESCRIPTION
        This function updates the properties of an existing Group Policy Object. It retrieves the
        current configuration, applies the specified changes, and submits the full object back to
        the API via PUT. Use Get-WEMGroupPolicyObject with -IncludeRegOperations when piping an
        object that includes registry operations.
    .PARAMETER Id
        The unique ID of the Group Policy Object to update.
    .PARAMETER InputObject
        A Group Policy Object (from Get-WEMGroupPolicyObject) to be modified. Can be passed via
        the pipeline. Use -IncludeRegOperations on Get-WEMGroupPolicyObject to ensure the full
        registry operations are included when updating them.
    .PARAMETER Name
        The new name for the Group Policy Object.
    .PARAMETER Description
        The new description for the Group Policy Object.
    .PARAMETER Enabled
        Enables or disables the Group Policy Object.
    .PARAMETER RegistryOperations
        A new array of registry operation objects (from New-WEMGroupPolicyRegistryItem) to replace
        the existing operations.
    .PARAMETER PassThru
        If specified, the command returns the updated Group Policy Object.
    .EXAMPLE
        PS C:\> Get-WEMGroupPolicyObject -IncludeRegOperations | Where-Object { $_.Name -eq "My GPO" } | Set-WEMGroupPolicyObject -Enabled $false

        Disables the Group Policy Object named "My GPO".
    .EXAMPLE
        PS C:\> Set-WEMGroupPolicyObject -Id 7 -Name "Renamed GPO" -Description "Updated description"

        Renames the Group Policy Object with ID 7 and updates its description.
    .NOTES
        Version:        1.0
    ...
    param(
        ...
        [Parameter(Mandatory = $false)]
        [PSCustomObject[]]$RegistryOperations,
    ...
    process {
        try {
            ...
            if ($PSCmdlet.ParameterSetName -eq 'ByInputObject') {
                ...
                Write-Verbose "Retrieving current settings for Group Policy Object with ID '$($Id)'..."
                $CurrentSettings = Get-WEMGroupPolicyObject -IncludeRegOperations | Where-Object { $_.id -eq $Id }
                if (-not $CurrentSettings) {
                    throw "A Group Policy Object with ID '$($Id)' could not be found in the active or specified site."
                }
            }

            $TargetDescription = "WEM Group Policy Object '$($CurrentSettings.Name)' (ID: $($CurrentSettings.id))"
            if ($PSCmdlet.ShouldProcess($TargetDescription, "Update")) {
                ...
                # Ensure REG_MULTI_SZ entries are arrays before sending (API returns them as strings)
                foreach ($Op in $CurrentSettings.registryOperations) {
                    if ($Op.regType -eq "REG_MULTI_SZ" -and $Op.regData -isnot [array]) {
                        $Op.regData = [array]$Op.regData
                    }
                }

                # The API expects the entire object in the body for a PUT request.
                $UriPath = "services/wem/groupPolicyObject"
                Invoke-WemApiRequest -UriPath $UriPath -Method "PUT" -Connection $Connection -Body $CurrentSettings

                if ($PassThru.IsPresent) {
                    Write-Verbose "PassThru specified, retrieving updated Group Policy Object..."
                    Write-Output (Get-WEMGroupPolicyObject | Where-Object { $_.id -eq $CurrentSettings.id })
                }
            }
        } catch {
            Write-Error "Failed to update WEM Group Policy Object: $($_.Exception.Message)"
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCADWnJMzc8Z8y8E
# f8HP9oG05kZWe/CwIII54tIv+BWf9aCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
