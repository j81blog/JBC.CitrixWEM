function Remove-WEMGroupPolicyRegistryItem {
    <#
    .SYNOPSIS
        Removes a registry operation from an existing WEM Group Policy Object in memory.
    .DESCRIPTION
        This function removes registry operation(s) matching the specified Key, ValueName, and
        Scope from a Group Policy Object's operations array. It modifies the object in memory
        and returns the updated GPO - no API call is made.
    ...
    .PARAMETER Key
        The registry key path of the operation to remove (e.g., "Software\Company\Settings").
    .PARAMETER ValueName
        The registry value name of the operation to remove.
    .PARAMETER Scope
        The scope of the operation to remove. Valid values: Machine, User.
    .EXAMPLE
        PS C:\> Get-WEMGroupPolicyObject -IncludeRegOperations | Where-Object Name -eq "My GPO" |
        PS C:\>     Remove-WEMGroupPolicyRegistryItem -Key "Software\Company" -ValueName "OldSetting" -Scope Machine |
        PS C:\>     Set-WEMGroupPolicyObject

        Removes the specified registry operation from "My GPO" and saves it.
    .NOTES
        Version:        1.0
    ...
    process {
        $Before = @($InputObject.registryOperations).Count
        $InputObject.registryOperations = @($InputObject.registryOperations | Where-Object {
            -not ($_.key -eq $Key -and $_.value -eq $ValueName -and $_.scope -eq $Scope)
        })
        $Removed = $Before - @($InputObject.registryOperations).Count

        if ($Removed -eq 0) {
            Write-Warning "No registry operation found matching Key '$($Key)', ValueName '$($ValueName)', Scope '$($Scope)'. No changes made."
        } else {
            Write-Verbose "Removed $($Removed) operation(s) matching Key '$($Key)\$($ValueName)' (Scope: $($Scope))."
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBGV7YEHOAsojz+
# UnLPHJe/fE+nc+VN348HipI1tEKCUqCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IDz27z38mW2iTqq8p8Q2hcLscNsx50LpcTu6BQOrDOu1MA0GCSqGSIb3DQEBAQUA
# BIIBgMfGOV9Q41BlxdSoLcQk7F6dF752Fx3RmaftfapHXH57KY7kg7da2ICxgk7X
# JCwbWtTtff9LaxaEc4mJ9jLbezz8aaapcJGcvXAENtMIBpCcvf8a8g8DZG5JJOkp
# AsicKFJjIYaDBhw9/VNJKXFTcwk1Ayl2Alo9AO15MeuGKg6RsSP5hbJx/fo7sKPi
# SblatmoiNbhlH6JscfUFD2qrpCtNJ13dN4XRagcgyV/zWXxbT+qdZ+dBjS9b1P+v
# tHo3ZGzCuVm9E3rDI+6GBTp+rb+Xn8/zZVYCnxdfzUJvRszxDcdZll6wLpxbK79n
