function Add-WEMGroupPolicyRegistryItem {
    <#
    .SYNOPSIS
        Adds a registry operation to an existing WEM Group Policy Object in memory.
    .DESCRIPTION
        This function adds a new registry operation to a Group Policy Object's operations array.
        It modifies the object in memory and returns the updated GPO -- no API call is made.
    ...
    .PARAMETER Action
        The registry operation to perform. Valid values: CreateKey, SetValue, DeleteValue,
        DeleteAllValues, DeleteKey.
    .PARAMETER Scope
        The registry hive scope. Valid values: Machine (HKLM), User (HKCU).
    .PARAMETER Key
        The registry key path (e.g., "Software\Company\Settings").
    .PARAMETER ValueName
        The name of the registry value. Leave empty for key-level operations.
    .PARAMETER RegType
        The registry value type. Required when -Action is 'SetValue'.
    .PARAMETER RegData
        The data for the registry value. Required when -Action is 'SetValue'.
    .EXAMPLE
        PS C:\> Get-WEMGroupPolicyObject -IncludeRegOperations | Where-Object Name -eq "My GPO" |
        PS C:\>     Add-WEMGroupPolicyRegistryItem -Action SetValue -Scope Machine -Key "Software\Company" -ValueName "Setting" -RegType REG_SZ -RegData "Value" |
        PS C:\>     Set-WEMGroupPolicyObject

        Adds a new registry operation to "My GPO" and saves it.
    .NOTES
        Version:        1.0
    ...
    process {
        if ($Action -eq "SetValue") {
            if ([string]::IsNullOrEmpty($RegType)) {
                Write-Error "The -RegType parameter is required when -Action is 'SetValue'."
                return $InputObject
            }
            if ($null -eq $RegData) {
                Write-Error "The -RegData parameter is required when -Action is 'SetValue'."
                return $InputObject
            }
        }

        if ($Action -eq "SetValue") {
            $NewItem = New-WEMGroupPolicyRegistryItem -Action $Action -Scope $Scope -Key $Key -Value $ValueName -Type $RegType -Data $RegData
        } else {
            $NewItem = New-WEMGroupPolicyRegistryItem -Action $Action -Scope $Scope -Key $Key -Value $ValueName
        }

        # Ensure existing REG_MULTI_SZ entries in the object are arrays (API returns them as strings)
        foreach ($ExistingOp in $InputObject.registryOperations) {
            if ($ExistingOp.regType -eq "REG_MULTI_SZ" -and $ExistingOp.regData -isnot [array]) {
                $ExistingOp.regData = [array]$ExistingOp.regData
            }
        }

        $InputObject.registryOperations = @($InputObject.registryOperations) + $NewItem
        Write-Verbose "Added '$($Action)' operation for key '$($Key)\$($ValueName)' (Scope: $($Scope))."
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDbY9EwzueiEECB
# BXDRPlNL+zGMHr0fPIqEmcpxEh4VpqCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IG20V9LDlNVbJnqFPMhF2UoQVBmX7AM1t81wsu7w0/0VMA0GCSqGSIb3DQEBAQUA
# BIIBgGI73tfLZt11Z3lydz5czq/xWlKNH7WIiKzXAJJTL7cXcuvNlvaz4W0mIO/o
# i1EI3J1X+hsIDkPkJtauj+UzKI0LJm2YWePEb+yxmDmuPPArkIFhJevQVQMHsZip
