function New-WEMUserDsn {
    <#
    .SYNOPSIS
        Creates a new User DSN action in a WEM Configuration Set.
    .DESCRIPTION
        This function adds a new User DSN (Data Source Name) action. If -SiteId is not specified, it uses
        the active Configuration Set defined by Set-WEMActiveConfigurationSite.
    .PARAMETER Name
        The unique name for the User DSN action.
    .PARAMETER DsnName
        The name of the Data Source Name (DSN) to be created.
    .PARAMETER ServerName
        The server name for the DSN connection.
    .PARAMETER DatabaseName
        The database name for the DSN connection.
    .PARAMETER Description
        An optional description for the User DSN action.
    .PARAMETER Enabled
        Whether the action is enabled. Defaults to $true.
    .PARAMETER RunOnce
        If specified, the action will only run once per user session.
    .PARAMETER UsingExtraCredential
        If specified, additional credentials will be used for the DSN connection.
    .PARAMETER UserName
        The username for DSN authentication (requires UsingExtraCredential).
    .PARAMETER UserPassword
        The password for DSN authentication (requires UsingExtraCredential).
    .PARAMETER SiteId
        The ID of the WEM Configuration Set. Defaults to the active site.
    .PARAMETER PassThru
        If specified, the command returns the newly created User DSN object.
    .EXAMPLE
        PS C:\> New-WEMUserDsn -Name "Sales DB" -DsnName "SalesData" -ServerName "SQL01" -DatabaseName "SalesDB"

        Creates a new User DSN action named "Sales DB" that creates a DSN called "SalesData".
    .NOTES
        Version:        1.0
    ...
    try {
        ...
        if ($PSCmdlet.ShouldProcess($DsnName, "Create WEM User DSN '$($Name)' in Site ID '$($ResolvedSiteId)'")) {
            $Body = @{
                actionType           = "CreateOrModifyDsn"
                ...
                runOnce              = $RunOnce.ToBool()
                usingExtraCredential = $UsingExtraCredential.ToBool()
                userName             = $UserName
            ...
            if ($PassThru.IsPresent) {
                Write-Verbose "PassThru specified, retrieving newly created User DSN..."
                # Use the unique Name property for a reliable lookup.
                $Result = Get-WEMUserDsn -SiteId $ResolvedSiteId | Where-Object { $_.Name -ieq $Name }
            }

            Write-Output ($Result | Expand-WEMResult -ErrorAction SilentlyContinue)
        }
    } catch {
        Write-Error "Failed to create WEM User DSN '$($Name)': $($_.Exception.Message)"
        return $null
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA+GpsF372Q7KTA
# mKcBOZXk0tF2XCq182gS5Xrw2lxUf6CCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IIC3WP/Mz/3RveEItDiBkvxTnzboEWV9xM488ip257YbMA0GCSqGSIb3DQEBAQUA
# BIIBgCQ1Wubf8OdleqfcvmQj4RWQ+CgamSsbvsxrpGtUPBL6S4WPkW7IYgeQrf+a
# BXvUPISrs/qceJCQ1DBSNhIrRibF/mPydWfjPCprEfvhpVHOvoIpwynd+LKy4oek
# sZuhVrgtaJMWYpW1onHyfLHdaD1Di+j28igyA06LB2kFJKZTaHqdO3AqeiJvkw4q
