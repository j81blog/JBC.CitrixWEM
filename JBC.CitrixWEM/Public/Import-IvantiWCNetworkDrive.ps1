function Import-IvantiWCNetworkDrive {
    <#
    .SYNOPSIS
        Imports Ivanti Workspace Control network drives into Citrix WEM.
    .DESCRIPTION
        Reads network drive configurations from an Ivanti Workspace Control Building Block XML file
        using Get-IvantiWCNetworkDrive, creates a WEM Network Drive action for each entry via
        New-WEMNetworkDrive, and assigns it to the appropriate AD targets via
        New-WEMNetworkDriveAssignment.

        Disabled network drives are skipped. AD users and groups are resolved from the WEM active
    ...
    .PARAMETER GUI
        If specified, opens a grid view to allow selection of which network drives to import.
    .EXAMPLE
        Import-IvantiWCNetworkDrive -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1

        Imports all enabled network drives from the building block into Configuration Set 1.
    .EXAMPLE
        Import-IvantiWCNetworkDrive -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1 -GUI

        Opens a selection grid before importing, letting you choose which network drives to process.
    .NOTES
        Function  : Import-IvantiWCNetworkDrive
    ...
    if ($ActiveDomain = Get-WEMActiveDomain) {
        ...
        foreach ( $Item in $itemsToProcess ) {
            ...
            try {
                ...
                try {
                    $WEMNetworkDrive = New-WEMNetworkDrive @Params -SiteId $ResolvedSiteId -PassThru -ErrorAction Stop
                ...
                foreach ($WEMAssignment in $Item.WEMAssignments) {
                    ...
                    $WEMNetworkDriveAssignment = New-WEMNetworkDriveAssignment -SiteId $ResolvedSiteId -Target $WEMAssignmentTarget -NetworkDrive $WEMNetworkDrive -PassThru @WEMAssignmentParams
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDu7q3EXiCCe7nz
# oPTZPMpN+p0IgIHJAiLnJ+qMfYxKvaCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IMapfMtk6PEuYbjZ1K/V03nILU+OYEGVPAg5wbDWirfEMA0GCSqGSIb3DQEBAQUA
# BIIBgEd5Iflww46pJWbAa2MZEyBWsNx52KeY6esIny8SurUVO6W1mA71FvHrVWav
# mv/zk8Cv3glGusAqhaCnG694yf2lVa80gfuNRjupCfvG1lPocravNSTRUZrc6g+O
# pNwZfVQ+uxypJd8HwdjuCVYLN5Sqw8cfCXWZ/VEuB5458xHKEtLNgltPGjU0JcM0
# 8D9WSK5k9dZdR6InPJSYG6aPWgaUZuf9qgtiGnxuLWtBv0uUF3IN6XrIzfYpGVF2
# Uo+g2dqwXuLHmQXcRDJDFawo57ExKmLWmFasOSmZr6DayucbVSUhujMYiVC6sprC
