function New-WEMGroupPolicyRegistryItem {
    <#
    .SYNOPSIS
        Creates a registry operation object for use with New-WEMGroupPolicyObject.
    .DESCRIPTION
        This helper function builds a PSCustomObject representing a single registry operation
        to be included in the -RegistryOperations parameter of New-WEMGroupPolicyObject.
    .PARAMETER Action
    ...
    .PARAMETER ValueName
        The name of the registry value. Required for SetValue and DeleteValue. Leave empty
        for key-level operations (CreateKey, DeleteKey, DeleteAllValues).
    .PARAMETER RegType
        The registry value type. Required when -Action is 'SetValue'. Valid values:
        REG_SZ, REG_DWORD, REG_DWORD_LITTLE_ENDIAN, REG_QWORD, REG_QWORD_LITTLE_ENDIAN,
        REG_MULTI_SZ, REG_BINARY, REG_EXPAND_SZ, REG_NONE.
    .PARAMETER RegData
        The data for the registry value. Required when -Action is 'SetValue'.
        - REG_SZ, REG_EXPAND_SZ: provide a [string]
        - REG_DWORD, REG_DWORD_LITTLE_ENDIAN: provide an [int]
        - REG_QWORD, REG_QWORD_LITTLE_ENDIAN: provide a [string] (number as string)
        - REG_MULTI_SZ: provide a [string[]] (array of strings)
        - REG_BINARY, REG_NONE: provide a [string] (base64-encoded)
    .EXAMPLE
        New-WEMGroupPolicyRegistryItem -Action SetValue -Scope Machine -Key "Software\Company" -ValueName "Setting" -RegType REG_SZ -RegData "Value"

        Creates a SetValue operation for a REG_SZ entry under HKLM.
    .EXAMPLE
        New-WEMGroupPolicyRegistryItem -Action CreateKey -Scope Machine -Key "Software\Company\NewKey"

        Creates a CreateKey operation under HKLM.
    .EXAMPLE
        New-WEMGroupPolicyRegistryItem -Action SetValue -Scope User -Key "Software\Company" -ValueName "Lines" -RegType REG_MULTI_SZ -RegData @("line1","line2")

        Creates a SetValue operation for a REG_MULTI_SZ entry under HKCU.
    .NOTES
        Version:        1.0
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCRoQo+My3mo33/
# R3AEBj4sg1SCRdAik/bVcOh33/psL6CCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ICQjHG2kIPXBdhQTiqGER2mpLGUkbwQ4DwtmLNDS53PNMA0GCSqGSIb3DQEBAQUA
# BIIBgLs4z4Z7fSb7qZKsFCxzOKo10Nll+WZ7qb7ceQIEOHIvuACOXgj+MXM5EsbC
# nojWaC9fjj/Svcyp2RnArXwe1E31yC+o6atF6pKhzSygzmH/ZBYaPW3snyoYSqXW
# UjqholhcuJhg47vrsRUdrQsROyISxADqV+zC7PGmfmlyMML3rlrkPstS1kc82JUV
# b/GwPB/Z20u8FURWsw5zNMYrtMyoX7QVUBbfbITRNYOXBSqjbP57u/pulopGwKjx
