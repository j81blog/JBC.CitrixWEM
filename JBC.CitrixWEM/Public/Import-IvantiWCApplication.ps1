function Import-IvantiWCApplication {
    <#
    .SYNOPSIS
        Imports Ivanti Workspace Control applications into Citrix WEM.
    .DESCRIPTION
        Reads application configurations from an Ivanti Workspace Control Building Block XML file
        using Get-IvantiWCApplication, creates a WEM Application action for each entry via
        New-WEMApplication, and assigns it to the appropriate AD targets via
        New-WEMApplicationAssignment.

        Disabled applications are skipped. AD users and groups are resolved from the WEM active
    ...
    .PARAMETER GUI
        If specified, opens a grid view to allow selection of which applications to import.
    .EXAMPLE
        Import-IvantiWCApplication -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1

        Imports all enabled applications from the building block into Configuration Set 1.
    .EXAMPLE
        Import-IvantiWCApplication -XmlFilePath 'C:\temp\LAB-BB.xml' -SiteId 1 -GUI

        Opens a selection grid before importing, letting you choose which applications to process.
    .NOTES
        Function  : Import-IvantiWCApplication
    ...
    if ($ActiveDomain = Get-WEMActiveDomain) {
        ...
        foreach ( $Item in $itemsToProcess ) {
            ...
            try {
                ...
                try {
                    $WEMApplication = New-WEMApplication @Params -SelfHealing $true -SiteId $ResolvedSiteId -PassThru -ErrorAction Stop
                ...
                foreach ($WEMAssignment in $Item.WEMAssignments) {
                    ...
                    $WEMApplicationAssignment = New-WEMApplicationAssignment -SiteId $ResolvedSiteId -Target $WEMAssignmentTarget -Application $WEMApplication -PassThru @WEMAssignmentParams
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDN6TwAuKjSHAnI
# kw0eVOOV0p5nSu2ana3+qUUC7lggmqCCIiowggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# IIB+983nOiRosF9Bbm0LkJRrtDIRJefpIoBQ5VToLiu4MA0GCSqGSIb3DQEBAQUA
# BIIBgI687RuAc+AbxVvwmhZkXPTbgmJhd/FE1dBXJV/+jO07EYTfgZmcXyGGOBq+
# 1ih3yHxMAe/1GapzxK4jD1incTCTjZfvBgFeycRSIPYvgtj32+nzYSjU7Un4lwUM
# 7FWbucPO5FVJTsIAuz1QvJZGvDQjMOXeN3BP7r8QbVwZn2mdqQI5hyoLyKjLRGKc
# l54Uk3zI1v9u5oFv0m8tj7BdUED3dvxxyiTL2cL0ZX93gVL/muw5PIFHoQwbxVRQ
# F1JYbwlL/iSirZiKJXWxyjAkjP6+5o79bOoySo/1SfQurdDyraXNRWeVFhl5h/2k
# rJFeeQ0bDnz1HkXGkmbOpCMalnvEiSWgihN5Kt2At8pI5uDgxuFz1H2h/gH2+YgZ
# CgM1otsqIDnPr2qXIohTPXROxcK1Y3cM8VaW8BRLrM1N/1hF/htSq8tfsJi6a8Wj
# fGryxwPSzhydimoaDpVBoELo6iDMPQNTe+o8EbhoduEqtBh6wZpCqQc28hzdZXiA
# YHjwgaGCFLIwghSuBgorBgEEAYI3AwMBMYIUnjCCFJoGCSqGSIb3DQEHAqCCFIsw
# ghSHAgEDMQ8wDQYJYIZIAWUDBAIBBQAwggFqBgsqhkiG9w0BCRABBKCCAVkEggFV
# MIIBUQIBAQYKKwYBBAGEWQoDATAxMA0GCWCGSAFlAwQCAQUABCCkBKyU3TWFTfs6
# 6qwtSEJuVrxsZ+X577gG9rS5Sk3d6AIGacZiOPEXGBMyMDI2MDQxNTE5NDY1OC41
...
# AQkEMSIEIMduiEl3MsfUeAy7N0sFrqgaRupVQ91Vq5axfxXuaEMGMIHdBgsqhkiG
...
# 71jXHZnzdbdaJdn9+I9RtQU3Kk+LLlkwDQYJKoZIhvcNAQELBQAEggIAOmsFVyZg
# PnkcbQeXXQ5n9iOfCYIGdKnO1kO7C/voNpVIiY9vF4Y3M2BJ37FaENuxpXZlguPO
# fKyUKtbMZ+TQ9N1wRK76fEkji2j1f/DpqVql3pxdoZLjoJEeMvVFev+Gx+p9ykOn
# gmAlSWSUSKv/e6CAGQQpuYrymuzLXyhEWHD71z36iniWI4pwHaUDnFAu9/l/1M6A
# DkwIcp9/0mmQNM72wogzYu9CeaCxwjtDtsbVw1k3W9TLxxOWrpXuI7yzm5W6OYhu
# s9mWbPCkerdKW3cJfNqZTR45h15sWLS3lhZjpXNjEui2IMKw0VNOPkdA950XIo8C
# M2p+MM614r2WBN3ILtw2AaB+KGUakeTafdbqjIxhoRYEaLvoseDK7Kg4UFjcekOs
# X/ISlu3qScUpqt33xjnjINQMjEbPX0U9lss4T7gOV5Z327ooags8+OlMdddApRZZ
# 5MU+6OnoMvSrmSo4qbgpH8LZSuYp1LIcvDo9IFjehXxBRlTyRhGf85xXDWoHNP7T
# MLIu2a/OT+dcafwAaf3ive2hLyueHDVlhT7sK5LA9BtwyM0gyJDJaXhq9SdxaoMu
# VcTRz/FWQZyHWzUv7o2Jtl7m+hI04b5Uu2Bc6iN+vV0nDctcqnZYve5rDawrUUnD
# lq1hWOntNvt41WYGMgN/nTDk/JWicR7QQ9c=
